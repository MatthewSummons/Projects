...
print("Coefficients for Angular Velocity Fit (a, b):", popt)
best_fit_y = sqrt(x, *popt)


series_fit_w_sqr = Polynomial.fit(x_data, y_sqr, 1)
print("Coefficients for Squared Velocity Frequency Fit (a, b):", series_fit_w_sqr.coef)
best_fit_y_sqr = series_fit_w_sqr(x)


# Figure 1
plt.figure("Angular Velocity on Height", figsize=(10, 6))
...
plt.legend(title='Angular Velocity on Height')

# Figure 2
plt.figure("Squared Angular Velocity on Height", figsize=(10, 6))
...
plt.xlabel("Vertical Height (m)")
plt.ylabel("Squared Angular Velocity ($rad^2s^{-2}$)")
plt.legend(title='Squared Angular Velocity on Height')
